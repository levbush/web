...
print_response('GET все пользователи', requests.get(BASE))

# GET один пользователь (корректный)
print_response('GET пользователь id=1', requests.get(f'{BASE}/1'))

# GET несуществующий пользователь (ошибка 404)
print_response('GET пользователь id=9999 - ожидаем 404', requests.get(f'{BASE}/9999'))
...
print_response('POST дублирующийся email - ожидаем ошибку', requests.post(BASE, json={**new_user, 'surname': 'Clone'}))

# DELETE созданного пользователя (корректный)
if new_id:
    print_response(f'DELETE пользователь id={new_id}', requests.delete(f'{BASE}/{new_id}'))
...
print_response('DELETE пользователь id=9999 - ожидаем 404', requests.delete(f'{BASE}/9999'))
